...
function deserializeObject(buffer, index, options, isArray) {
  ...
  while (!done) {
    ...
    if (elementType === constants.BSON_DATA_STRING) {
      ...
      object[name] = promoteValues ? value : new Int32(value);
      index += 4;
    } else if (elementType === constants.BSON_DATA_NUMBER) {
      const value = buffer.readDoubleLE(index);
      object[name] = promoteValues ? value : new Double(value);
      index = index + 8;
      ...
      if (raw) {
        ...
        object[name] = deserializeObject(buffer, index, options, false);
      }
      ...
      object[name] = deserializeObject(buffer, index, arrayOptions, true);
      index = index + objectSize;
      ...
      let binarySize = readInt32LE(buffer, index);
      index += 4;
      ...
      if (subType === Binary.SUBTYPE_BYTE_ARRAY) {
        binarySize = readInt32LE(buffer, index);
        index += 4;
        if (binarySize < 0)
    ...
    } else if (elementType === constants.BSON_DATA_REGEXP) {
      // Get the start search index
      i = index;
      // Locate the end of the c string
      while (buffer[i] !== 0x00 && i < buffer.length) {
      ...
      if (bsonRegExp === false) {
        ...
        // Parse options
        for (let i = 0; i < regExpOptions.length; i++) {
      ...
      const totalSize = readInt32LE(buffer, index);
      index += 4;
      ...
      const scopeObject = deserializeObject(buffer, index, options, false);
      // Adjust the index
      index = index + objectSize;
